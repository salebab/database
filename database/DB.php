...
class DB extends PDO {
    ...
    /**
     * Use DB::getInstance() instead create new instace
     */
    function __construct() {
        try {
            parent::__construct('mysql::host=' . DB_HOST . ';dbname=' . DB_NAME, DB_USER, DB_PASS, array(PDO::ATTR_STATEMENT_CLASS => array("DBStatement", array($this)), PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES UTF8", PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION));
    ...
    private function removeNonExistentColumns($table, &$data, $prev_column_id = null) {
        if($this->check_columns) {
            ...
            if(!empty($prev_column_id) && empty($this->prev_columns[$prev_column_id])) {
                $columns = $this->prev_columns[$prev_column_id] = $this->getColumnsFromTable($table);
            }
    ...
    function insert($table, $data, $prev_stmt_id = null) {
        ...
        elseif(empty($this->prev_stmt[$prev_stmt_id])) {
            $stmt = $this->prev_stmt[$prev_stmt_id] = $this->getInsertStmt($table, $data);
        }
        // use previous stmt
        else {
    ...
    static function createQuery() {
        ...
        return new DBQuery(DB::getInstance());
    }
    ...
    function getColumnsFromTable($table, $table_alias = "", $use_delimiter = "_", $caching = false) {
		...
		*/
        ...
        if (!$columns) {
            $sql = "DESCRIBE $table";
            $r = DB::getInstance()->executeQuery($sql);
            $columns = $r->fetchAll(DB::FETCH_COLUMN);
    ...
    function save($table, $data, $column_id_name, $prev_stmt_id = null) {
        ...
        if ($column_id > 0) {
            DB::getInstance()->update($table, $data, $column_id_name . " = ?", $column_id, $prev_stmt_id);
        } else {
            DB::getInstance()->insert($table, $data, $prev_stmt_id);
        }
    }
}
